...
def tex_bin_dirs():
    ...
    nobody should have to know: `x86_64-linux`, `aarch64-linux`, and so on. Glob
    for it rather than guessing. A cluster node has no administrator, so a TeX
    under `$HOME` comes first and a system one is a bonus.
    """
    candidates = []
