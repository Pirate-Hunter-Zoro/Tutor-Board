...
check("the newest question is what a turn answers",
      turns.newest_question(repo) == "0001")

# --- a turn, and then a correction of it ------------------------------------
turns.write_turn(repo, {"id": "t0001", "rev": 1, "kind": "ink", "answers": "0001",
                        "t": 1000.0, "png": "/answers/t0001-r1.png",
                        "ink": "/answers/t0001-r1.json", "strokes": 9})
sent = turns.load_turns(repo)
check("a sent answer is a turn", len(sent) == 1 and sent[0]["id"] == "t0001")
check("a turn records the card it answers", sent[0]["answers"] == "0001")

check("the next turn gets the next id", turns.next_turn_id(repo) == "t0002")
check("revising the same turn bumps its revision",
      turns.turn_revision(repo, "t0001") == 2)

turns.write_turn(repo, {"id": "t0001", "rev": 2, "kind": "ink", "answers": "0001",
                        "t": 9000.0, "png": "/answers/t0001-r2.png",
                        "ink": "/answers/t0001-r2.json", "strokes": 11})
sent = turns.load_turns(repo)
check("a correction supersedes rather than piling up", len(sent) == 1)
check("the newest revision is the one shown", sent[0]["rev"] == 2)
check("but it keeps its place in the transcript", sent[0]["t0"] == 1000.0)
...
turns.write_turn(repo, {"id": "t0002", "rev": 1, "kind": "text", "answers": "0001",
...
check("and the high-water mark is on disk, outside the transcript",
      os.path.isfile(os.path.join(repo.live, turns.TURN_SEQ)))
...
check("nor does pruning the inbox rewind it",
      turns.next_turn_id(repo) == "t0003")

# And it keeps advancing.
turns.write_turn(repo, {"id": "t0003", "rev": 1, "kind": "ink", "answers": "0001",
                        "t": 1.0, "from": "student", "strokes": 1})
check("a new turn advances it", turns.next_turn_id(repo) == "t0004")
check("a fresh id starts at revision 1", turns.turn_revision(repo, "t0004") == 1)
check("and a live turn's next write is a revision, not an overwrite",
      turns.turn_revision(repo, "t0003") == 2)
